...
export default {
   ...
   * @param id 主键id
   */
  content: (id: string) => new GetRequestModel<string>(`/opm/template/detail/content/${id}`).request(),

  /**
   * 新增模板内容
   * @param data 新增的数据
   */
  add: (data: TemplateDetailDto) => new PostRequestModel<TemplateDetailVo>('/opm/template/detail', data).request(),
   ...
   * @param data 修改的数据
   */
  update: (id: string, data: TemplateDetailDto) => new PutRequestModel<TemplateDetailVo>(`/opm/template/detail/${id}`, data).request(),
